...
if (isset($_POST['insert_complete'])) {
    ...
    try {
        if ($table === 'Batiment' && isset($_POST['nomBat'], $_POST['nomGest'], $_POST['mdpGest'])) {
            $sql = "INSERT INTO `Batiment` (`NomBat`, `NomGest`, `MDPGest`) VALUES (:nomBat, :nomGest, :mdpGest)";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':nomBat', $_POST['nomBat']);
            ...
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':nomCap', $_POST['nomCap']);
